...
<script type="module">
  import { doc, setDoc, getDoc, onSnapshot } from "https://www.gstatic.com/firebasejs/10.13.1/firebase-firestore.js";
  ...
  function enableRealtimeSync() {
    // Listen to the same document; when it changes, update local state and UI
    onSnapshot(doc(window.db, "appData", "batikAir"), (snap) => {
      if (snap.exists()) {
        const data = snap.data();
        if (data.tasks) allTasks = data.tasks;
        if (data.attendanceRecords) attendanceRecords = data.attendanceRecords;
  ...
  document.getElementById('signInBtn').addEventListener('click', async () => {
    ...
    if (!id || !name || !role) { alert('Please fill all login fields'); return; }
    ...
    // Load and then enable realtime sync
    await loadSavedData();
    enableRealtimeSync();
    renderTasks();
    updateProdDatePill();
  ...
  function renderTasks() {
    ...
    const sections = { all: [], assigned: [], done: [] };
    allTasks.forEach((t, i) => {
      const matchesSearch = !search || JSON.stringify(t).toLowerCase().includes(search);
  ...
  function populateATAFilter() {
    ...
    if (!container) {
      ...
      document.querySelector('.container').insertBefore(d, document.getElementById('allTasks'));
      container = d;
    }
